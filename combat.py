...
def battle(player, opponent):
    ...
    while((player.stats["Health"] > 0) and (opponent.health > 0)):
        ...
        #and vice versa, opponent may be able to attack twice or more before player attacks
            ...
            #regen at end of turn (stamina, mana)
            end_turn_regen(player)
    ...
    if((player.stats["Health"] > 0) and (opponent.health <= 0)):
        ...
        end_battle_regen(player)
        return True # end of battle
...
#regen health at end of battle according to regen stat
def end_battle_regen(player):
...
# regen mana and stamina at end of turn according to regen stat
def end_turn_regen(player):
...
def player_attack(player, opponent):
    ...
    while(not valid_attack_choice):
        print("\n") # spacing
        print("{:<3}{:<15}{:<3}".format("#", "Skill", "Cost"))
        for i in range(len(player.skills)): #-1 for indexing
            if(player.skills[i].mana_cost > 0):
                print("{:<3}{:<15}{:<3} Mana. ".format(str(i+1), player.skills[i].name.capitalize(), player.skills[i].mana_cost))
            elif(player.skills[i].stamina_cost > 0):
                print("{:<3}{:<15}{:<3} Stamina.".format(str(i+1), player.skills[i].name.capitalize(), player.skills[i].stamina_cost))
            else:
                print("{:<3}{:<15}{:<3}".format(str(i+1), player.skills[i].name.capitalize(), "None"))
        print("\n")
        ...
        if((int(attack_choice)-1) <= len(player.skills) ):
            #use the skill
           valid_attack_choice = use_skill(player, opponent, attack_choice)
        else:
...
#apply the skill
def use_skill(player, opponent, attack_choice):
     #get skill
